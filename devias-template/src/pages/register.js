...
const Page = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers) => {
      try {
        const response = await apiHandler.handleSignup(values.email, values.password, values.firstName, values.lastName);
        ...
        <Card elevation={16}>
          ...
          <CardContent>
            ...
            >
              <Stack spacing={3}>
                <TextField
                  error={!!(formik.touched.firtName && formik.errors.firstName)}
                  fullWidth
